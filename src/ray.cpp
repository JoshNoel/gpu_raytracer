...
namespace cray
{
	...
	__device__ Ray Ray::make_primary(unsigned x, unsigned y, const Camera& p_camera) {
		...
		/*
		imagePlanePos.z = p_camera.m_focal_length;
		imagePlanePos.x = p_camera.m_world_width * (float(x) / float(p_camera.m_width) - 0.5f);
		imagePlanePos.y = p_camera.m_world_height *  (float(y) / float(p_camera.m_height) - 0.5f);
		*/
